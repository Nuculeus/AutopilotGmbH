...
describe("POST /api/paperclip/workspace-api/agents", () => {
  beforeEach(() => {
    vi.resetModules();
    vi.clearAllMocks();
    vi.unstubAllGlobals();
  ...
  it("autobinds an existing openai secret when a compatible agent is created later", async () => {
    ...
    const response = await POST(
      new Request("http://localhost/api/paperclip/workspace-api/agents", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          name: "CEO",
      ...
      { params: Promise.resolve({ path: ["workspace-api", "agents"] }) },
    );

    expect(response.status).toBe(201);
    expect(fetchMock).toHaveBeenCalledTimes(3);
    expect(fetchMock.mock.calls[1]?.[0]).toBe("http://paperclip:3100/api/companies/cmp_123/secrets");
